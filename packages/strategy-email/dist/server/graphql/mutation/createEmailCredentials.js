...
import { Credential, Authority, ForbiddenError, ConflictError, NotFoundError, ValidationError, Role, validateIdFormat, DataLoaderExecutor, } from "@authx/authx";
...
export const createEmailCredentials = {
    type: new GraphQLList(GraphQLEmailCredential),
    description: "Create a new credential.",
    args: {
        credentials: {
            type: new GraphQLNonNull(new GraphQLList(new GraphQLNonNull(GraphQLCreateEmailCredentialInput))),
        },
    },
    async resolve(source, args, context) {
        const { executor: { strategies, connection: pool }, authorization: a, realm, base, sendMail, } = context;
        if (!a) {
        ...
        const results = args.credentials.map(async (input) => {
            ...
            try {
                ...
                const authority = await Authority.read(tx, input.authorityId, strategies, { forUpdate: true });
                ...
                const existingCredentials = await EmailCredential.read(executor, (await tx.query(`
          ...
          FOR UPDATE
          `, [authority.id, input.email])).rows.map(({ id }) => id));
                ...
                if (!(await a.can(executor, realm, createV2AuthXScope(realm, {
                    ...
                    const { proof } = input;
                    if (proof) {
                        if (!authority.details.publicKeys.some((key) => {
                            try {
                                ...
                                if (payload.email !== input.email) {
                                    throw new ForbiddenError("This proof was generated for a different email address.");
                                }
                                // Make sure this is for the same user
                                if (payload.sub !== a.userId) {
                                    throw new ForbiddenError("This proof was generated for a different user.");
                                }
                                return true;
                            }
                            catch (error) {
                    ...
                    // users, so she needs to prove control of the email address.
                        ...
                        await sendMail({
                            ...
                            text: substitute({ proof, url }, authority.details.verificationEmailText),
                            html: substitute({ proof, url }, authority.details.verificationEmailHtml),
                        });
                        throw new ForbiddenError("An email has been sent to this address with a code that can be used to prove control.");
                    }
                }
                // Disable the conflicting credential
                if (existingCredentials.length === 1) {
                    await EmailCredential.write(tx, {
                ...
                const credential = await EmailCredential.write(tx, {
                    id,
...
//# sourceMappingURL=createEmailCredentials.js.map