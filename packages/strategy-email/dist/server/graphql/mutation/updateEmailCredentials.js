...
import { Credential, ForbiddenError, NotFoundError, ValidationError, validateIdFormat, DataLoaderExecutor, } from "@authx/authx";
...
export const updateEmailCredentials = {
    type: new GraphQLList(GraphQLEmailCredential),
    description: "Update a new credential.",
    args: {
        credentials: {
            type: new GraphQLNonNull(new GraphQLList(new GraphQLNonNull(GraphQLUpdateEmailCredentialInput))),
        },
    },
    async resolve(source, args, context) {
        const { executor: { strategies, connection: pool }, authorization: a, realm, } = context;
        if (!a) {
            throw new ForbiddenError("You must be authenticated to update an credential.");
        }
        ...
        const results = args.credentials.map(async (input) => {
            ...
            try {
                ...
                if (!(await before.isAccessibleBy(realm, a, executor, {
                    ...
                    throw new ForbiddenError("You do not have permission to update this credential.");
                }
...
//# sourceMappingURL=updateEmailCredentials.js.map