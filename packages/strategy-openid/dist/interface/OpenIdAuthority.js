...
const mutationFn = async ({ authorityId, code, }) => {
...
export function OpenIdAuthority({ authority, setAuthorization, }) {
    ...
    async function onSubmit(e) {
        ...
        window.localStorage.setItem(`authx.a.${authority.id}.r`, window.location.href);
        // Strip all search params except authorityId from the redirect URL.
        const redirect = new URL(window.location.href);
        new URL(window.location.href).searchParams.forEach((v, k) => k !== "authorityId" && redirect.searchParams.delete(k));
    ...
    return (React.createElement("form", { onSubmit: onSubmit, className: errors.length ? "panel validate" : "panel" },
        ...
        React.createElement("label", null,
            React.createElement("input", { disabled: mutation.isPending, ref: focusElement, type: "submit", value: mutation.isPending
                    ? "Loading..."
