...
import { v4 } from "uuid";
import { GraphQLAuthorization, Authority, Authorization, User, Role, ForbiddenError, AuthenticationError, DataLoaderExecutor, } from "@authx/authx";
...
export const authenticateOpenId = {
    ...
    async resolve(source, args, context) {
        ...
        try {
            ...
            if (!(authority instanceof OpenIdAuthority)) {
                throw new AuthenticationError("The authority uses a strategy other than openid.");
            }
            // Fetch the ID token.
            ...
            requestBody.append("redirect_uri", `${base}?authorityId=${args.authorityId}`);
            ...
            if (!credential &&
                ...
                const emailAuthority = await EmailAuthority.read(executor, authority.details.emailAuthorityId);
                const emailCredential = emailAuthority &&
                    emailAuthority.enabled &&
                    (await emailAuthority.credential(executor, token.email));
                if (emailCredential && emailCredential.enabled) {
                    credential = await OpenIdCredential.write(tx, {
                        enabled: true,
                        id: v4(),
            ...
            if (!credential && authority.details.createsUnmatchedUsers) {
                const user = await User.write(tx, {
                    id: v4(),
                    enabled: true,
                    type: "human",
                    name: token.name || "",
                ...
                if (authority.details.assignsCreatedUsersToRoleIds &&
                    authority.details.assignsCreatedUsersToRoleIds.length) {
                    const roles = await Promise.all(authority.details.assignsCreatedUsersToRoleIds.map((id) => Role.read(executor, id)));
                    const roleResults = await Promise.allSettled(roles.map((role) => Role.write(tx, {
                        ...role,
            ...
            if (!isSuperset(await user.access(executor, values), createV2AuthXScope(realm, {
                type: "authorization",
                authorizationId: "",
                grantId: "",
                clientId: "",
                ...
                throw new ForbiddenError("You do not have permission to create this authorization");
            }
            // Create a new authorization.
            const authorization = await Authorization.write(tx, {
                id: authorizationId,
                enabled: true,
...
//# sourceMappingURL=authenticateOpenId.js.map