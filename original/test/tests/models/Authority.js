	...
	describe('(static methods)', () => {
		describe('query', () => {
			...
			it('accepts a query tranformation argument', async () => {
				var authoritys = await Authority.query(conn, q =>
					q.filter({ strategy: 'google' })
				);
				assert.isArray(authoritys);
		...
		describe('create', () => {
			...
			it('should create a new object', async () => {
				var authority = await Authority.create(conn, {
		...
		describe('save', () => {
			...
			it('should save a new object', async () => {
				var authority = await Authority.save(conn, 'test-static-save', {
					id: 'test-static-save',
					name: 'Saved New Authority',
					strategy: '2factor'
				});
				assert.isString(authority.id);
			...
			it('should save an existing object', async () => {
				var authority = await Authority.update(conn, 'test-static-save', {
					id: 'test-static-save',
					name: 'Saved Existing Authority',
					strategy: '2factor'
				});
		...
		describe('update', () => {
			...
			it('should update the correct object', async () => {
				var authority = await Authority.update(conn, ids[0], {
		...
		describe('credentials', () => {
			it('should return all assigned credentials', async () => {
				...
				credentials.forEach(credential =>
					assert.instanceOf(credential, Credential)
				);
			});
		});
