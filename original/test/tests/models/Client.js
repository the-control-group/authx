	...
	describe('(static methods)', () => {
		describe('query', () => {
			...
			it('accepts a query tranformation argument', async () => {
				var clients = await Client.query(conn, q =>
					q.filter({ name: 'Dunder Mifflin Inventory Portal' })
				);
				assert.isArray(clients);
		...
		describe('create', () => {
			...
			it('should create a new object', async () => {
				var client = await Client.create(conn, {
					name: 'Created Client',
					secret:
						'05687eac667178f54c2974795925a1519bd3ad5b384b93b0af8a3495e2ba78bf'
				});
		...
		describe('save', () => {
			...
			it('should save a new object', async () => {
				var client = await Client.save(conn, 'test-static-save', {
					id: 'test-static-save',
					name: 'Saved New Client',
					secret:
						'd240e3049355259b49b902e2d7559c572add7b6264378cddd231ad9ca3004500'
				});
				assert.isString(client.id);
			...
			it('should save an existing object', async () => {
				var client = await Client.save(conn, 'test-static-save', {
					id: 'test-static-save',
					name: 'Saved Existing Client',
					secret:
						'3eb7de2d16504aa4fcf19bc71082d6711ea2266316b16d9e5582b6a170430c0e'
				});
		...
		describe('update', () => {
			...
			it('should update the correct object', async () => {
				var client = await Client.update(conn, ids[0], {
					name: 'Updated Client'
				});
	...
	describe('(instance methods)', () => {
		before(async () => {
			var client = await Client.create(conn, {
				name: 'Created Client',
				secret:
					'aeb20b23eb74a51dfbb910b4c9742dd019c02925de3d17bc89566c659c991852'
			});
			assert.isString(client.id);
			ids.push(client.id);
		});
