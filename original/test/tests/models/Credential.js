	...
	describe('(static methods)', () => {
		describe('query', () => {
			...
			it('accepts a query tranformation argument', async () => {
				var credentials = await Credential.query(conn, q =>
					q.filter({
						details: {
							password:
								'$2a$04$GM8OJ7/Oq4H2Q.d9Yk3Ga.ffKmrUez7EYTHmEoX7jHpkDmmepl1/W'
						}
					})
				);
				assert.isArray(credentials);
		...
		describe('create', () => {
			...
			it('should reject a mismatched profile ID', async () => {
				try {
					await Credential.create(conn, {
						id: ['email', 'support@dundermifflin.com'],
						user_id: 'eaa9fa5e-088a-4ae2-a6ab-f120006b20a9',
						details: { token: 'Created Credential' },
						profile: { id: 'foo', displayName: 'Dunder Mifflin Support' }
					});
				} catch (err) {
					assert.instanceOf(err, errors.ValidationError);
					return;
			...
			it('should create a new object without a profile', async () => {
				var credential = await Credential.create(conn, {
					id: ['email', 'help@dundermifflin.com'],
					user_id: 'eaa9fa5e-088a-4ae2-a6ab-f120006b20a9',
					details: { token: 'Created Credential' }
				});
				...
				assert(
					...
					'Expected `created` timestamp to be after ' + time
				);
				assert(
					credential.last_updated >= time,
					'Expected `last_updated` timestamp to be after ' + time
				);
			...
			it('should create a new object with a profile', async () => {
				var credential = await Credential.create(conn, {
					...
					details: { token: 'Created Credential' },
					profile: {
						id: 'support@dundermifflin.com',
						displayName: 'Dunder Mifflin Support'
					}
				});
				assert.isArray(credential.id);
				assert.equal(credential.id[0], 'email');
		...
		describe('get', () => {
			...
			it('should fetch the correct object', async () => {
				var credential = await Credential.get(conn, [
					'email',
		...
		describe('save', () => {
			...
			it('should save a new object', async () => {
				var credential = await Credential.save(
					conn,
					['email', 'frontdesk@dundermifflin.com'],
					{
						id: ['email', 'frontdesk@dundermifflin.com'],
						user_id: 'eaa9fa5e-088a-4ae2-a6ab-f120006b20a9',
						details: { token: 'Saved New Credential' },
						profile: {
							id: 'frontdesk@dundermifflin.com',
							displayName: 'Dunder Mifflin Support'
						}
					}
				);
				assert.deepEqual(credential.id, [
					'email',
					'frontdesk@dundermifflin.com'
				]);
			...
			it('should save an existing object', async () => {
				var credential = await Credential.save(
					...
					['email', 'frontdesk@dundermifflin.com'],
					{
						id: ['email', 'frontdesk@dundermifflin.com'],
						user_id: 'eaa9fa5e-088a-4ae2-a6ab-f120006b20a9',
						details: { token: 'Saved Existing Credential' },
						profile: {
							id: 'frontdesk@dundermifflin.com',
							displayName: 'Dunder Mifflin Support'
						}
		...
		describe('update', () => {
			...
			it('should update the correct object', async () => {
				var credential = await Credential.update(conn, ids[0], {
					details: { token: 'Updated Credential' }
				});
