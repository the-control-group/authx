	...
	describe('(static methods)', () => {
		describe('query', () => {
			...
			it('accepts a query tranformation argument', async () => {
				var users = await User.query(conn, q =>
					q.filter({ profile: { displayName: 'Toby Flenderson' } })
				);
				assert.isArray(users);
		...
		describe('create', () => {
			...
			it('should create a new object', async () => {
				try {
					var user = await User.create(conn, {
						profile: { displayName: 'Created User' },
						type: 'human'
					});
				} catch (e) {
					console.log(e);
		...
		describe('update', () => {
			...
			it('should update the correct object', async () => {
				var user = await User.update(conn, ids[0], {
