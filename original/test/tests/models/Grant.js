...
describe('Grant', () => {
	...
	describe('(static methods)', () => {
		describe('query', () => {
			...
			it('accepts a query tranformation argument', async () => {
				var grants = await Grant.query(conn, q =>
					q.filter({ nonce: 'd122298d-55d9-4eee-9c17-463113669007' })
				);
				assert.isArray(grants);
		...
		describe('create', () => {
			...
			it('should create a new object', async () => {
				var grant = await Grant.create(conn, {
					id: [
						'1691f38d-92c8-4d86-9a89-da99528cfcb5',
						'dundermifflin-inventory'
					],
					scopes: ['a:b:c']
				});
		...
		describe('get', () => {
			...
			it('should fetch the correct object', async () => {
				var grant = await Grant.get(conn, [
					'a6a0946d-eeb4-45cd-83c6-c7920f2272eb',
					'dundermifflin-inventory'
				]);
				assert.deepEqual(grant.id, [
					'a6a0946d-eeb4-45cd-83c6-c7920f2272eb',
					'dundermifflin-inventory'
				]);
			});
	...
	describe('(instance methods)', () => {
		before(async () => {
			var grant = await Grant.create(conn, {
				id: ['306eabbb-cc2b-4f88-be19-4bb6ec98e5c3', 'dundermifflin-inventory'],
				scopes: ['a:b:c']
			});
			assert.isArray(grant.id);
			ids.push(grant.id);
