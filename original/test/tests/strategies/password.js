...
describe('Strategy: Password', () => {
	...
	it('should accept a form-encoded body', done => {
		agent
			...
			.send(
				'password=123456&username=password&username=a6a0946d-eeb4-45cd-83c6-c7920f2272eb'
			)
			.expect(200)
			.end(done);
	});
	...
	it('should accept HTTP basic credentials', done => {
		agent
			.get('/v1/session/password')
			.set(
				'Authorization',
				'Basic WyJlbWFpbCIsIm1pY2hhZWwuc2NvdHRAZHVuZGVybWlmZmxpbi5jb20iXToxMjM0NTY='
			)
			.expect(200)
			.end(done);
	});
