...
module.exports.query = async function query(ctx) {
	...
	roles = await Promise.filter(roles, r =>
	...
	// apply SCIM filters
	if (filter) roles = roles.filter(filter.test);
...
module.exports.get = async function get(ctx) {
	await protect(
		ctx,
		ctx[x].authx.config.realm + ':role.' + ctx.params.role_id + ':read'
	);
...
module.exports.patch = async function patch(ctx) {
	await protect(
		ctx,
		ctx[x].authx.config.realm + ':role.' + ctx.params.role_id + ':update'
	);
	...
	await Role.update(ctx[x].conn, ctx.params.role_id, {
