...
module.exports = async ctx => {
	...
	if (ctx.query.response_type === 'code') {
		...
		if (!requestedScopes.every(s => scopeRegex.test(s)))
			throw new errors.ValidationError(
				'The `scope` parameter must be a space-separated list of AuthX scopes.'
			);
		...
		if (!ctx[x].session || ctx[x].session.sub !== ctx[x].user.id) {
			ctx.redirect(
				ctx[x].authx.config.routes.login +
					(ctx[x].authx.config.routes.login.includes('?') ? '&' : '?') +
					'url=' +
		...
		if (
			!requestedScopes.every(rS =>
				authorizedScopes.some(aS => scopes.can(aS, rS))
			)
		) {
			...
			if (ctx.method === 'POST') {
				try {
					...
					throw new errors.ValidationError(
						'Authorized `scopes` must be a json-encoded array of scopes.'
					);
				}

				userAuthorizedScopes = data.scopes;
				authorizedScopes = scopes.simplifyCollection(
					authorizedScopes.concat(userAuthorizedScopes)
				);
			}
			...
			// look for a previous grant issued to this client by the user
				...
				try {
					...
					if (err instanceof errors.NotFoundError) return requestApproval(ctx);
		...
		let nonce = uuid.v4();
		let code = new Buffer(JSON.stringify([ctx[x].user.id, nonce])).toString(
			'base64'
		);
		...
		ctx.redirect(
			ctx.query.redirect_uri +
				(ctx.query.redirect_uri.includes('?') ? '&' : '?') +
				'code=' +
				code +
				'&state=' +
				ctx.query.state
	...
	// The client exchanged an authorization code for an access token.
		...
		// HTTP POST (json)
		if (ctx.is('application/json')) data = await json(ctx.req);
		// HTTP POST (form)
		...
		if (!data.client_secret)
			throw new errors.ValidationError(
				'A valid `client_secret` must be provided.'
			);
		...
		if (data.grant_type === 'authorization_code') {
			...
			grant = await Grant.getWithNonce(
				ctx[x].conn,
				[code[0], data.client_id],
				code[1]
			);
		}
		...
		// ### Refresh Token
			...
			if (!data.refresh_token)
				throw new errors.ValidationError(
					'A valid `refresh_token` must be provided.'
				);

			try {
				refresh_token = new Buffer(data.refresh_token, 'base64').toString(
					'utf8'
				);
				...
				throw new errors.ValidationError(
					'The provided `refresh_token` is invalid.'
				);
			}
			...
			if (grant.refresh_token !== refresh_token[1])
				throw new errors.ValidationError(
					'The provided `refresh_token` is incorrect.'
				);
		} else
			throw new errors.ValidationError(
		...
		let authorizedScopes = scopes.simplifyCollection(
			globallyAuthorizedScopes.concat(grant.scopes)
		);

		// all scopes (grant x user) authorized in the token
		let totalScopes = scopes.combineCollections(
			authorizedScopes,
			await user.scopes()
		);
		...
		ctx.body = {
			...
			refresh_token: new Buffer(
				JSON.stringify([user.id, grant.refresh_token])
			).toString('base64'),
		...
		throw new errors.ValidationError(
			'Missing `response_type=code` query parameter or POST body.'
		);
};
...
function requestApproval(ctx) {
	...
	ctx.redirect(
		ctx[x].authx.config.routes.authorize +
			(ctx[x].authx.config.routes.authorize.includes('?') ? '&' : '?') +
			'url=' +
			url +
			'&scope=' +
			scope
