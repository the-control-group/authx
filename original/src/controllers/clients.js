...
module.exports.query = async function query(ctx) {
	...
	ctx.body = await Promise.filter(clients, c =>
		can(ctx, ctx[x].authx.config.realm + ':client.' + c.id + ':read')
	);
};
...
module.exports.get = async function get(ctx) {
	await protect(
		ctx,
		ctx[x].authx.config.realm + ':client.' + ctx.params.client_id + ':read'
	);
...
module.exports.patch = async function patch(ctx) {
	await protect(
		ctx,
		ctx[x].authx.config.realm + ':client.' + ctx.params.client_id + ':update'
	);
	var data = await json(ctx.req);
...
module.exports.del = async function del(ctx) {
	await protect(
		ctx,
		ctx[x].authx.config.realm + ':client.' + ctx.params.client_id + ':delete'
	);
