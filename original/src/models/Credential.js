...
module.exports = class Credential extends Model {
	...
	static async create(conn, data) {
		...
		if (
			...
			data.id[1] &&
			data.profile &&
			typeof data.profile.id === 'undefined'
		)
		...
		if (err)
			throw new errors.ValidationError(
				'A valid credential must be supplied.',
				err.validation
			);
		if (data.profile && data.profile.id !== data.id[1])
			throw new errors.ValidationError(
				'If a profile ID is present, it must match the `authority_user_id`.'
			);
	...
	static async save(conn, id, data) {
		...
		if (!Array.isArray(data.id) || data.id.some((v, i) => v !== id[i]))
	...
	async authority(refresh) {
		...
		if (!this[AUTHORITY] || refresh)
			this[AUTHORITY] = models.Authority.get(
