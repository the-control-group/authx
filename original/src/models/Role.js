...
module.exports = class Role extends Model {
	...
	async users(refresh) {
		...
		if (!this[USERS] || refresh) {
			let assignments = Object.keys(this.assignments).filter(
				k => this.assignments[k]
			);
			this[USERS] = models.User.query(this[Model.Symbols.CONN], q =>
				q.getAll(r.args(assignments))
			);
		}
