...
module.exports = class Grant extends Model {
	...
	static async create(conn, data) {
		...
		if (err)
			throw new errors.ValidationError(
				'A valid grant must be supplied.',
				err.validation
			);
	...
	static async getWithNonce(conn, id, nonce) {
		...
		var result = await r
			...
			.update(
				row =>
					r.branch(row('nonce').eq(nonce), row.merge({ nonce: null }), row),
		...
		if (!result.replaced)
			throw new errors.NotFoundError(
				'The requested grant does not exist, or the authorization code has already been used.'
			);
