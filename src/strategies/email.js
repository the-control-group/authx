	...
	async authenticate(ctx) {
		...
		if (request.token) {
			...
			if (token.sub[0] === 'credential') {
				...
				if (ctx[x].user && (ctx[x].user.id !== credential.user_id))
					throw new errors.AuthenticationError('This email is associated with a different user.');
			...
			if (token.sub[0] === 'user') {
				...
				if (!user)
					throw new errors.AuthenticationError('A new credential cannot be created because no user is currently logged in.');


				// verify that the logged-in user matches the intended user
				if (user.id !== tokenUserId)
					throw new errors.AuthenticationError('A new credential cannot be created because the logged-in user does not match the intended user.');
			...
			throw new errors.AuthenticationError('The supplied token subject is not supported.');
		}
		...
		// and send the user a link by email
			...
			try {
				...
				await this.mail({
					to: request.email,
					subject: Handlebars.compile(this.authority.details.authentication_email_subject)(templateContext),
					text: Handlebars.compile(this.authority.details.authentication_email_text)(templateContext),
					html: Handlebars.compile(this.authority.details.authentication_email_html)(templateContext)
				});
				...
				await this.mail({
					to: request.email,
					subject: Handlebars.compile(this.authority.details.verification_email_subject)(templateContext),
					text: Handlebars.compile(this.authority.details.verification_email_text)(templateContext),
					html: Handlebars.compile(this.authority.details.verification_email_html)(templateContext)
				});
