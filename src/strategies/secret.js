...
export default class SecretStrategy extends Strategy {
	...
	async authenticate(ctx) {
		...
		if (!user_id)
			ctx.throw(400, 'The `user_id` must be specified.');


		// validate the secret
		var secret = request.secret;
		if (!secret)
			ctx.throw(400, 'The `secret` must be specified.');
	...
	async createCredential(data) {
		...
		// hash the secret
		data.details.secret = await hash(data.details.secret, this.authority.details.rounds);
	...
	async updateCredential(credential, delta) {
		...
		if (delta.details.secret)
			delta.details.secret = await hash(delta.details.secret, this.authority.details.rounds);
